const contractAddress = "0x68B1D87F95878fE05B998F19b66F4baba5De1aed"; // Replace with the deployed contract address
const abi = [
	{
		"inputs": [],
...
async function mintNFT() {
    ...
    try {
        await sendTransaction(contractAddress, abi, 'safeMint', [selectedWallet, uri], selectedWallet);
...
// View metadata of an NFT
async function viewMetadata() {
    const tokenId = document.getElementById('tokenId').value;

    try {
        const metadata = await callContractMethod(contractAddress, abi, 'tokenURI', [tokenId]);
        document.getElementById('metadataResult').textContent = metadata;
    } catch (error) {
...
async function transferNFT() {
    ...
    try {
        await sendTransaction(contractAddress, abi, 'safeTransferFrom', [selectedWallet, recipient, tokenId], selectedWallet);
...
// Load the initial address and update UI
window.onload = () => {
